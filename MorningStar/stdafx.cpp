// stdafx.cpp : •W€ƒCƒ“ƒNƒ‹[ƒh MorningStar.pch ‚Ì‚Ý‚ð
...
void ApplicationUnhandledException(Object^ sender, UnhandledExceptionEventArgs^ e)
{
	...
	if(!MTINFO.SERVER_MODE){
		MessageBox::Show("A unhandled exception occurred in the MorningStar thread and has been written to the error.log file.\n\nMorningStar will attempt to recover.", "Unhandled Exception", MessageBoxButtons::OK, MessageBoxIcon::Error);
		try {
			return;
		}catch (Exception^ ex){
			MessageBox::Show("MorningStar will now exit.", "Recovery Failed", MessageBoxButtons::OK, MessageBoxIcon::Error);
...
void LoadMTOption()
{
	...
	TCHAR* iniSection = _T("MorningStar");
	UINT iniVersion;
	_stprintf_s(ini, _T("%sMorningStar.ini"), MTOPTION.PATH);
	// stdafx.h‚É‹Lq
	...
	if(_tcslen(MTOPTION.PROFILE) == 0){
		GetPrivateProfileString(iniSystem, _T("DefaultProfile"), _T("MorningStar"),  MTOPTION.PROFILE,        MAX_ARRAY,   ini);
	}
	...
	GetPrivateProfileString(iniSystem, _T("Name"),               _T("No Username"),  MTOPTION.NAME,           MAX_NAME,    ini);
	GetPrivateProfileString(iniSystem, _T("Comment"),            _T(""),            MTOPTION.COMMENT,        MAX_NAME,    ini);
	GetPrivateProfileString(iniSystem, _T("Looking"),            _T("A worthy opponent."),            MTOPTION.LOOKING,        MAX_NAME,    ini);
	GetPrivateProfileString(iniSystem, _T("Avoiding"),           _T("Lag."),                         MTOPTION.AVOIDING,        MAX_NAME,    ini);
	GetPrivateProfileString(iniSystem, _T("Region"),             _T("North America"),            MTOPTION.REGION,        MAX_NAME,    ini);
	GetPrivateProfileString(iniSystem, _T("GG"),                 _T("gg"),            MTOPTION.GG,        MAX_NAME,    ini);
	MTOPTION.SHOW_REGION			    = GetPrivateProfileInt(iniSystem, _T("ShowRegion"),          0, ini);
	...
	MTOPTION.GG_ENABLE				    = GetPrivateProfileInt(iniSystem, _T("EnableGG"),       0, ini) == 1 ? true : false;
	MTOPTION.LEGACY_SERVER				= GetPrivateProfileInt(iniSystem, _T("LegacyNetcode"),       0, ini) == 1 ? true : false;
	MTOPTION.DARK_MODE				    = GetPrivateProfileInt(iniSystem, _T("DarkMode"),       0, ini) == 1 ? true : false;
	MTOPTION.SYS_TRAY				    = GetPrivateProfileInt(iniSystem, _T("SysTray"),       0, ini) == 1 ? true : false;
	...
	try{
		...
		MessageBox::Show("Failed to load bookmark information.\n", "Bookmark loading", MessageBoxButtons::OK, MessageBoxIcon::Error);
	}
...
void SaveMTOption()
{
	...
	WritePrivateProfileString(iniSystem, _T("Region"),               MTOPTION.REGION,               ini);
	WritePrivateProfileString(iniSystem, _T("GG"),					 MTOPTION.GG,               ini);

	_itot_s(MTOPTION.SYS_TRAY, buf, 10);				WritePrivateProfileString(iniSystem, _T("SysTray"), buf, ini);
	_itot_s(MTOPTION.DARK_MODE, buf, 10);				WritePrivateProfileString(iniSystem, _T("DarkMode"), buf, ini);
	_itot_s(MTOPTION.GG_ENABLE, buf, 10);				WritePrivateProfileString(iniSystem, _T("EnableGG"), buf, ini);
	_itot_s(MTOPTION.SHOW_REGION, buf, 10);				WritePrivateProfileString(iniSystem, _T("ShowRegion"), buf, ini);
	...
	_itot_s(MTOPTION.LEGACY_SERVER, buf, 10);			WritePrivateProfileString(iniSystem, _T("LegacyNetcode"), buf, ini);
	...
	_itot_s(MTOPTION.REST_CONNECT, buf, 10);			WritePrivateProfileString(iniSystem, _T("RestConnect"), buf, ini);
...
String^ EncryptionIP(String^ ip)
{
	...
	try {
		...
		return "•ÏŠ·Ž¸”sBBase64‚Ê‚é‚ÛƒGƒ‰[";
	}
	catch (FormatException^) {
		return "•ÏŠ·Ž¸”sBBase64Œ`Ž®ƒGƒ‰[";
	}
	return ipBase64;
}
...
void UPnP_PortOpenClose(bool s, bool auto_close){ /* open:close, auto:manual */
	...
	try{
		...
		if(s){	// ŠJ•ú
			...
			MessageBox::Show(String::Format("UPnP Enabled on the Port: {0}\n\nThis will allow certain types of online play. This port can temporarily only be used by MorningStar.", port), mestitle, MessageBoxButtons::OK, MessageBoxIcon::Information);
			if(MTINFO.DEBUG){
